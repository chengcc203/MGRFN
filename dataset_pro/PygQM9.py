...
class QM9(InMemoryDataset):
    ...
    def process(self):
        ...
        mol_list, smiles_list = self.ab_feature()
        R = data['R']
        Z = data['Z']
        N = data['N']
        err=np.loadtxt('/dataset/qm9/raw/err.txt')
        ...
        for i in tqdm(valid_data):
            ...
            data = Data(pos=R_i, z=z_i, y=y_i[0], mu=y_i[0], alpha=y_i[1], homo=y_i[2], lumo=y_i[3], gap=y_i[4],
                        r2=y_i[5], zpve=y_i[6], U0=y_i[7], U=y_i[8], H=y_i[9], G=y_i[10], Cv=y_i[11], afe=a_fea, bfe=b_fea, mol = mol_list[j], smiles=smiles_list[j])
...
if __name__ == '__main__':
    ...
    split_idx = dataset.get_idx_split(len(dataset.data.y), train_size=700, valid_size=150,test_size=150, seed=42)
    print(split_idx)
    print(dataset[split_idx['train']])
    train_dataset, valid_dataset, test_dataset = dataset[split_idx['train']], dataset[split_idx['valid']], dataset[split_idx['test']]
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
    data = next(iter(train_loader)) #DataBatch(y=[32], pos=[388, 3], z=[388], mu=[32], alpha=[32], homo=[32], lumo=[32], gap=[32], r2=[32], zpve=[32], U0=[32], U=[32], H=[32], G=[32], Cv=[32], afe=[390, 7], bfe=[385, 8], batch=[388], ptr=[33])
    print(data)