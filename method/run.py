...
class run():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500,
            ...
            energy_and_force=False, p=100, save_dir='', log_dir='', train=True, name='aspirin', num=1000, modelName=''):
        r"""
        ...
        for epoch in range(1, epochs + 1):
            ...
            if (train):
                print('\nTraining...', flush=True)
                if energy_and_force:
                    train_mae, train_force = train_mae = self.train(model, optimizer, train_loader, energy_and_force, p,
                                                                    loss_func, device, name)
                else:
            ...
            if valid_mae < best_valid:
                ...
                if save_dir != '':
                    ...
                    torch.save(model.state_dict(), os.path.join(save_dir, name + '_valid_best_checkpoint.pt'))
            scheduler.step()

            file1 = open('mae/' + str(num) + '/mae_' + name + '_' + str(num) + modelName + '.txt', 'a')
