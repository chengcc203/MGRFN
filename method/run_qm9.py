...
class run_qm9():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=30, weight_decay=0, 
        ...
        for epoch in range(1, epochs + 1):
            ...
            if train==True:
                ...
                valid_mae, _, _ = self.val(model, valid_loader, energy_and_force, p, evaluation, device)
            

            print('\n\nTesting...', flush=True)
            test_mae, molecular_features, labels = self.val(model, test_loader, energy_and_force, p, evaluation, device)
            ...
            if valid_mae < best_valid:
                ...
                best_test = test_mae
                np.savez(
                    "/homec/shupengcheng/codes/CFFN/dataset/qm9/qm9_MGRFN_representations.npz",
                    features=np.concatenate(molecular_features, axis=0),
                    labels=np.concatenate(labels, axis=0),
                )
                if save_dir != '':
                    print('Saving checkpoint...')
                    torch.save(model.state_dict(), os.path.join(save_dir, str(tar) + '_valid_MGRFN_best_checkpoint.pt'))
           
            scheduler.step()
            file1=open('mae'+'/qm9'+'/mae_'+str(tar)+'.txt','a')
    ...
    def val(self, model, data_loader, energy_and_force, p, evaluation, device):
        ...
        return evaluation.eval(input_dict)['mae'], all_features, all_labels
